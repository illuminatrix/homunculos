#!/bin/bash
# Test: setsid syscall (SYS_setsid=66)
...
echo "  Subtest 3: setsid_test binary (setsid returns pid)"
echo "  Subtest 4: poweroff shuts down QEMU (regression)"

SETSID_TEST_BIN="$(dirname "$0")/setsid_test.elf"
dd if="$DISK_IMG" bs=512 skip=2048 of=.part.img 2>/dev/null
debugfs -w .part.img -R "write $SETSID_TEST_BIN /bin/setsid_test" 2>/dev/null
...
send_keys "setsid_test" 0.08
sleep 0.2
monitor_cmd "sendkey ret" >/dev/null 2>&1
sleep 3

if vga_contains "ALL TESTS PASS"; then
	pass "'setsid_test' produced 'ALL TESTS PASS'"
elif vga_contains "PASS"; then
	vga_raw=$(vga_text | tail -c 300)
	pass "'setsid_test' produced PASS output. VGA tail: [$vga_raw]"
elif vga_contains "FAIL"; then
	vga_raw=$(vga_text | tail -c 300)
	fail "'setsid_test' reported failure. VGA tail: [$vga_raw]"
	errors=$((errors + 1))
elif vga_contains "not found"; then
	fail "'setsid_test' binary not found on disk"
	errors=$((errors + 1))
else
	vga_raw=$(vga_text | tail -c 200)
	fail "'setsid_test' produced unexpected output. VGA tail: [$vga_raw]"
	errors=$((errors + 1))
fi
